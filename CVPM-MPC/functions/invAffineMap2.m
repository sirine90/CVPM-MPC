function Pnew = invAffineMap2(P, T, t)
% Compute the inverse image of the polyhedron under the affine map T*x+t
%   M = { x | T*x+t \in P }   (see Polyehdron.invAffineMap)
% can use non squrae T matrices
%
% Inputs:
% P: (Polyhedron) input Polyhedron, which is transformed
% T: (matrix) Transformation matrix (can be non-squeard)
% t: (vector) Transformation offset
%
% Output: 
% Pnew: (Polyhedron) transformed Polyhedron
